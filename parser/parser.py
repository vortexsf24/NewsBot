...
async def fetch(session: aiohttp.ClientSession, url: str) -> str:
    """
    Fetches the HTML content of the provided URL by making a request using the specified session.
    ...
    Returns:
        response: The HTML content of the webpage as a string.
    """
...
# Section responding for the asynchronous requests on news portals and scraping certain data from them.
...
async def parse_guardiansport() -> tuple:
    try:
        async with aiohttp.ClientSession() as session:
            ...
            articles = tuple(
                elem.text for elem in soup.findAll('a', class_='u-faux-block-link__overlay js-headline-text'))
            links = tuple(
...
async def parse_bbc() -> tuple:
    try:
        async with aiohttp.ClientSession() as session:
            ...
            articles = tuple(elem.text for elem in
                             soup.find('div', class_='gel-wrap gs-u-pt+').findAll('h3',
        ...
        async with aiohttp.ClientSession() as session:
            ...
            links = tuple(elem.get('href') for elem in
                          soup.find('div', class_='sc-17uq8ex-0 fakHlO').findAll('a',
                                                                                 class_='sc-1out364-0 dPMosf js_link')
                          if
...
async def main(config: Config) -> None:
    ...
    All news are being scraped and written into database every 5 minutes.
